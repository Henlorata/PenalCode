...
export function AppLayout() {
  ...
  // Eldöntjük, hogy meg kell-e jeleníteni a menüt
  const showSidebar = location.pathname !== '/onboarding';

  // --- ONBOARDING VÉDELEM ÉS IRÁNYÍTÁS ---
  useEffect(() => {
    if (loading || !profile) return;

    // 1. Ha Trainee ÉS még nincs kész az onboarding -> Kényszerítjük az /onboarding oldalra
    if (profile.faction_rank === 'Deputy Sheriff Trainee' && !profile.onboarding_completed) {
      if (location.pathname !== '/onboarding') {
        navigate('/onboarding', {replace: true});
      }
    }
    // 2. Ha NEM Trainee VAGY már kész van -> Tiltjuk az /onboarding oldalt
  ...
  return (
    <div className={cn("min-h-screen bg-slate-950 text-slate-100 flex transition-colors duration-700", statusClass)}>
      ...
      {/* SIDEBAR - Csak ha a showSidebar igaz */}
      {showSidebar && (
        <aside
              ...
              <DropdownMenuTrigger asChild>
                <button
                  ...
                  <div className="flex-1 min-w-0 text-left">
                    ...
                    <p
                      className="text-[10px] text-slate-500 truncate uppercase tracking-wide">{profile.faction_rank}</p>
                  </div>
      ...
      {/* MAIN CONTENT - Dinamikus margóval */}
      <main className={cn(
        "flex-1 flex flex-col min-h-screen transition-all duration-300 relative z-10",
        showSidebar ? "lg:ml-64" : "" // Ha nincs sidebar, nincs margó
      )}>

        {/* MOBIL HEADER - Csak ha a showSidebar igaz */}
        {showSidebar && (
          <>
            <header
